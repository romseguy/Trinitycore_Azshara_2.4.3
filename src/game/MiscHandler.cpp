...
void WorldSession::HandleWhoOpcode(WorldPacket & recv_data)
{
    ...
    for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
    {
        ...
        if (!(racemask & (1 << race)))
            continue;
        uint32 pzoneid;
		// to make it failproof should add something like if entered zonename contains "arena" "nagrand "blades" "edge" etc etc"
        if (itr->second->InArena() && itr->second->GetBattleGround()->isRated() && !(GetPlayer()->isGameMaster())) // if player is not a GM, dont show people in rated arena's
        {
