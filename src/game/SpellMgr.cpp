			...
			case SPELL_AURA_ADD_PCT_MODIFIER:
				{
					...
					case SPELLMOD_COST:                 // dependent from bas point sign (negative -> positive)
						if (spellproto->EffectBasePoints[effIndex]+int32(spellproto->EffectBaseDice[effIndex]) > 0)
						{
							if (spellproto->Id == 12042 || spellproto->Id == 36032)    // Arcane Power & Arcane Blast are positive spells
							    return true;
							return false;
						}
						break;
...
void SpellMgr::LoadSpellCustomAttr()
{
	...
	for (uint32 i = 0; i < GetSpellStore()->GetNumRows(); ++i)
	{
		...
		case 6277: // Bind Sight
			spellInfo->AttributesEx |= (SPELL_ATTR_EX_NOT_BREAK_STEALTH | SPELL_ATTR_EX_STACK_FOR_DIFF_CASTERS);
			mSpellCustomAttr[i] |= SPELL_ATTR_EX_IGNORE_LOS;
            spellInfo->AttributesEx3 |= SPELL_ATTR_EX3_DEATH_PERSISTENT;
			//count++; might be i++? not sure what to do with this
			break;
