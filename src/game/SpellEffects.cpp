        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_HUNTER:
            {
                ...
                else if (m_spellInfo->SpellFamilyFlags & 0x100000000LL)
                {
                    ...
                    damage += int32(float(base)/m_caster->GetAttackTime(RANGED_ATTACK)*2800 + m_caster->GetTotalAttackPowerValue(RANGED_ATTACK)*0.4f);
...
void Spell::EffectPowerBurn(uint32 i)
{
	...
	m_caster->SendSpellNonMeleeDamageLog(unitTarget, m_spellInfo->Id, new_damage, GetSpellSchoolMask(m_spellInfo), 0, 0, false, 0, false); // mana burn in combat log
}
    ...
    for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; ++i)
    {
        if (Spell* spell = unitTarget->GetCurrentSpell(CurrentSpellTypes(i)))
        {
                ...
                && curSpellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_INTERRUPT && curSpellInfo->PreventionType == SPELL_PREVENTION_TYPE_SILENCE)
            {
                ...
                unitTarget->InterruptSpell(CurrentSpellTypes(i), false,true,true); // I think this is the right spot, change: @@ -3665,7 +3665,7 @@" AND  @@ -4319,7 +4319,7 @@
            }
        }
    }
}
...
void Spell::EffectMomentMove(uint32 i)
{
    ...
    if (unitTarget->GetTypeId() == TYPEID_PLAYER)
        ...
        unitTarget->GetMap()->CreatureRelocation((Creature*)unitTarget, fx[itr_j], fy[itr_j], fz[itr_j] + 0.07531, orientation);
...
void Spell::EffectCharge(uint32 /*i*/)
{
    ...
    //m_caster->Relocate(x,y,z,m_caster->GetOrientation());
