...
void SocialMgr::GetFriendInfo(Player *player, uint32 friendGUID, FriendInfo &friendInfo)
{
        ...
        (pFriend->GetSession()->GetSecurity() == SEC_PLAYER || gmInWhoList && pFriend->IsVisibleGloballyFor(player))))
    {
		...
		if (pFriend->InArena() && !(player->isGameMaster())) // if player is not a GM, dont show people in rated arena's
        { // for some reason it wont allow me to check IfRated() here so we must leave it like this.
