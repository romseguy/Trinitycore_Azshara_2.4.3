...
pAuraHandler AuraHandler[TOTAL_AURAS]=
{
    ...
    &Aura::HandleAuraReflect,                               // 28 SPELL_AURA_REFLECT_SPELLS        implement in Unit::SpellHitResult
...
,m_tickNumber(0)
{
    ...
    m_isRemovedOnShapeLost = (m_caster_guid == m_target->GetGUID() && m_spellProto->Stances &&
                            !(m_spellProto->AttributesEx2 & 0x80000) && !(m_spellProto->Attributes & 0x10000) && !(spellproto->Id == 6788) && !(spellproto->Id == 12328));
}
    ...
    if (apply)
    {
        ...
        switch (GetId())
        {
            ...
            case 25046:
            {
                ...
                if (dummy)
                {
                    int32 amount = dummy->GetStackAmount();
                    if(amount > 3)
                        amount = 3;
                    int32 bp = amount * 10;
...
void Aura::HandleAuraReflect(bool apply, bool Real)
{
    if (!Real)
        return;
