...
bool PetAI::_needToStop() const
{
    ...
    if (!me->canSeeOrDetect(me->getVictim(), true))
        return true;

    if(me->IsFriendlyTo(me->getVictim()))
        return true;
    ...
    if(!me->GetCurrentSpell(CURRENT_CHANNELED_SPELL))
    {
        for (Unit::AuraMap::const_iterator itr = me->getVictim()->GetAuras().begin(); itr != me->getVictim()->GetAuras().end(); ++itr)
        {
            ...
            if (aura && (aura->GetSpellProto()->AuraInterruptFlags & AURA_INTERRUPT_FLAG_DAMAGE) && !aura->IsPositive()) // does the target has any (negative)aura which breaks on damage?
            {
                return true;
            }
        }
