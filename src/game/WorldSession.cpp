...
m_latency(0), m_timeOutTime(0), m_Warden(NULL), timeLastWhoCommand(0)
{
    if (sock)
    {
...
void WorldSession::LogoutPlayer(bool Save)
{
    ...
    if (_player)
    {
        ...
        else if (!_player->getAttackers().empty())
        {
			...
			if(kill) // had to do this because target dummy's might kill you on logout even though you are NOT in combat.
			{ // logout in pvp combat still kills you
