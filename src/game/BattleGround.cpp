...
void BattleGround::Update(time_t diff)
{
    ...
    { //BG is empty
        if(!GetInvitedCount(HORDE) && !GetInvitedCount(ALLIANCE)) // and noone in queue
            m_SetDeleteThis = true; // delete bg

        return; // bg empty but people still in queue, dont waste cpu on battleground events
    }
...
uint32 BattleGround::GetAlivePlayersCountByTeam(uint32 Team) const
{
    ...
    for (std::map<uint64, BattleGroundPlayer>::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        if (itr->second.Team == Team)
        {
            ...
            if (pl && pl->isAlive() && (!pl->HasAuraType(SPELL_AURA_SPIRIT_OF_REDEMPTION)))
                ++count;
        }
    }
    return count;
...
{
    return objmgr.GetClosestGraveYard(player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(), player->GetMapId(), player->GetBGTeam());
}
