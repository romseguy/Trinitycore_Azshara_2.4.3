...
void MovementInfo::Read(ByteBuffer &data)
{
    ...
    //if (HasMovementFlag(MOVEFLAG_HOVER) && HasMovementFlag(MOVEFLAG_JUMPING))
    //    moveFlags &= ~MOVEFLAG_JUMPING;
    // MOVEFLAG_JUMPING NOT USED?
...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss)
{
    ...
    // si le sort a pour but d'infliger des dommages
    if (damagetype != NODAMAGE)
    {
		// si les dommages sont abs et que la victime a deja un camouflage : on ne break pas le stealth
        if (!damage && pVictim->HasAuraType(SPELL_AURA_MOD_STEALTH))
			return 0;
			
        if (spellProto)
        {
            // si les dommages infliges par le spell (sans tenir compte de l'absorption) doivent casser des auras
            if (!(spellProto->AttributesEx4 & SPELL_ATTR_EX4_DAMAGE_DOESNT_BREAK_AURAS))
            {
                // on casse les auras correspondants
                pVictim->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_DAMAGE, spellProto->Id);
                pVictim->RemoveSpellbyDamageTaken(damage, spellProto->Id);
            }
        }
        else
        {
            pVictim->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_DAMAGE, 0);
            pVictim->RemoveSpellbyDamageTaken(damage, 0);
        }
    }
    ...
    // On donne de la rage a la victime qui a absorbe les dommages de l'attaquant
    if (!damage && cleanDamage && cleanDamage->damage)
    {
        ...
        // On n'inflige pas de dommages a la victime puisque tout a ete absorbe
       return 0;
	}
...
SpellMissInfo Unit::SpellHitResult(Unit *pVictim, SpellEntry const *spell, bool CanReflect, bool canMiss, bool WardCanReflect)
{
	...
	{	
		if (spell->Mechanic == MECHANIC_CONFUSED || spell->Mechanic == MECHANIC_FEAR || spell->Mechanic == MECHANIC_STUN || spell->Id == 2094 || spell->Id == 1833 || spell->Id == 8643 || spell->Id == 10308 || spell->Id == 10890 || spell->Id == 5246 || spell->Id == 17928) // Vanished Blind removes stealth 
		{
	...
	if (spell->Mechanic == MECHANIC_FEAR || spell->Mechanic == MECHANIC_STUN)
	{
		if (pVictim->HasAura(19574,0) || pVictim->HasAura(34471,0))
			return SPELL_MISS_IMMUNE;
	}
    ...
    // molten shields rank 1
        if(pVictim->HasSpell(11094) && (pVictim->HasAura(27128,1)))
        WardCanReflect = true;
    // molten shields rank 2
        if(pVictim->HasSpell(13043) && (pVictim->HasAura(27128,1)))
        WardCanReflect = true;
    ...
    if (WardCanReflect)
    {
        ...
        if(pVictim->HasAura(11189,0) || pVictim->HasSpell(11094))
            reflectchance = 10;
        // frost warding rank 2 && molten shields rank 2
        if(pVictim->HasAura(28332,0) || pVictim->HasSpell(13043))
            reflectchance = 20;
            ...
            if((pVictim->HasSpell(11094) || pVictim->HasSpell(13043)) && spell->SchoolMask == SPELL_SCHOOL_MASK_FIRE)
            {
                if (reflectchance > 0 && roll_chance_i(reflectchance))
                    return SPELL_MISS_REFLECT;
            }
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch (dummySpell->Id)
            {
                ...
                case 35429:
                {
                    ...
                    // In case of Whirlwind/Cleave
                    if (getVictim() != pVictim)
                        return false;
...
bool Unit::HandleOverrideClassScriptAuraProc(Unit *pVictim, Aura *triggeredByAura, SpellEntry const *procSpell, uint32 cooldown)
{
    ...
    switch(scriptId)
    {
        ...
        case 4537:                                          // Dreamwalker Raiment 6 pieces bonus
            ...
            break;
        case 37447:
	case 27103:                                          // Improved Mana Gems (Serpent-Coil Braid)
...
uint32 Unit::SpellHealingBonus(SpellEntry const *spellProto, uint32 healamount, DamageEffectType damagetype, Unit *pVictim)
{
    ...
    if (spellProto->SpellFamilyName == SPELLFAMILY_SHAMAN && spellProto->SpellFamilyFlags & 0x0000000000000040LL)
    {
       ...
       {
       if (GetTotalAttackPowerValue(BASE_ATTACK) > SpellBaseHealingBonus(GetSpellSchoolMask(spellProto)))
               heal += GetTotalAttackPowerValue(BASE_ATTACK) / 5;
       else
               heal += AdvertisedBenefit;
       }
...
void Unit::SetInCombatState(bool PvP, Unit* enemy)
{
    ...
    if (PvP)
        m_CombatTimer = 5700; // should be 6 seconds
            ...
            case SPELL_AURA_PRAYER_OF_MENDING:
                {
                    ...
                    if(!procSpell)
                        HandleMendingAuraProc(triggeredByAura); // melee attacks and such
                    else if(procSpell->Id == 27213 || procSpell->Id == 11684 || procSpell->Id == 11683 || procSpell->Id == 1948)
                        useCharges = false; // spells that should not proc POM, currently hellfire only
                    else
                        HandleMendingAuraProc(triggeredByAura); // normal case
                    break;
                }
...
Unit* Unit::SelectNearbyTarget(float dist) const
{
    ...
    {
        if (!IsWithinLOSInMap(*tIter) || ((*tIter)->ToCreature() && (*tIter)->ToCreature()->isTotem()) || (*tIter)->isSpiritService() || (*tIter)->GetCreatureType() == CREATURE_TYPE_CRITTER)
            targets.erase(tIter++);
        else
            ++tIter;
    }
