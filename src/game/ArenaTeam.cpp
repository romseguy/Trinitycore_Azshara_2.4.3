...
uint32 ArenaTeam::GetPoints(uint32 MemberRating)
{
    ...
    if (m_Type == ARENA_TEAM_2v2)
        points *= 1.00f; // slightly increased from old core
    else if (m_Type == ARENA_TEAM_3v3)
        points *= 1.50f; // still the same
...
bool ArenaTeam::IsQueued() const
{
    for (MemberList::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
        if (Player *p = objmgr.GetPlayer(itr->guid))
            if (p->InBattleGroundQueueForBattleGroundQueueType(BATTLEGROUND_QUEUE_2v2)
