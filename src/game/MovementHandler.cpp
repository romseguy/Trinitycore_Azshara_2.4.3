    ...
    if (plMover)
    {
        ...
        if (opcode == MSG_MOVE_FALL_LAND && !plMover->isInFlight())
        {
			...
			if (plMover->HasAuraType(SPELL_AURA_MOD_DECREASE_SPEED))
				skipAnticheat = true;
			if (plMover->HasAuraType(SPELL_AURA_MOD_INCREASE_SPEED))
				skipAnticheat = true;
			if (plMover->HasAuraType(SPELL_AURA_MOD_INCREASE_MOUNTED_SPEED))
				skipAnticheat = true;
	    ...
	    if(plMover->m_anti_skipspeed <= 0)
            ...
            // Anti-Flyhack
            if (((movementInfo.GetMovementFlags() & (MOVEFLAG_CAN_FLY | MOVEFLAG_FLYING | MOVEFLAG_FLYING2)) != 0) && !(plMover->HasAuraType(SPELL_AURA_FLY) || plMover->HasAuraType(SPELL_AURA_MOD_FLIGHT_SPEED_MOUNTED)) && GetSecurity() == SEC_PLAYER)
            {
                #ifdef MOVEMENT_ANTICHEAT_DEBUG
    ...
    if (plMover)                                            // nothing is charmed, or player charmed
    {
        if (check_passed || plMover->isGameMaster() || forcemovement)
        {
            ...
            if(movementInfo.HasMovementFlag(MOVEFLAG_MOVING) && GetPlayer()->HasUnitMovementFlag(MOVEFLAG_MOVING))
                plMover->InterruptSpellNotGrenade(CURRENT_GENERIC_SPELL,false,false,false);
