...
void WorldSession::HandleBattleGroundPVPlogdataOpcode(WorldPacket & /*recv_data*/)
{
    ...
    BattleGround *bg = _player->GetBattleGround();
    if (!bg || (bg->isArena() && bg->GetStatus() != STATUS_IN_PROGRESS)) //The WOTLK fix for scoreboard. hope i works -malaco
        return;
...
void WorldSession::HandleBattleGroundPlayerPortOpcode(WorldPacket &recv_data)
{
    ...
    if (_player->InBattleGroundQueue())
	{
		...
		{
			// if player tries to enter battleground (not arena) and has deserter, remove him from queue.
            if (!_player->CanJoinToBattleground())
            {
        ...
        switch(action)
        {
            ...
            case 0:                                         // leave queue
                ...
                */
                ...
                sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, bg, _player->GetTeam(), queueSlot, STATUS_NONE, 0, 0, arenaType);
