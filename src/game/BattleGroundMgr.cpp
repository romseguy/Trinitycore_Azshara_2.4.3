		...
		if(m_AutoDistributePoints)
	{
		if (m_AutoDistributionTimeChecker < diff)
		{
			if (time(NULL) > m_NextAutoDistributionTime)
			{
				...
				DistributionTime = fields[0].GetInt32();
			m_NextAutoDistributionTime = DistributionTime + BATTLEGROUND_ARENA_POINT_DISTRIBUTION_DAY * sWorld.getConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_INTERVAL_DAYS);
				CharacterDatabase.PQuery("UPDATE saved_variables SET NextArenaPointDistributionTime = '%u'", m_NextAutoDistributionTime);
				outstring_log ("NextArenaPointDistributionTime: %d",m_NextAutoDistributionTime);
			}
