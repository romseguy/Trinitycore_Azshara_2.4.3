...
, m_caster(Caster)
{
    ...
    if(m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MAGIC && (m_spellInfo->AttributesEx & 0x80)==0 && !IsAreaOfEffectSpell(m_spellInfo))
    {
        for (int j=0;j<3;j++)
        {
...
void Spell::prepareDataForTriggerSystem()
{
    ...
    else                              // Exceptions (some periodic triggers)
    {
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
			...
			if (m_spellInfo->SpellFamilyFlags & 0x0000000800200080LL && m_spellInfo->SpellIconID != 225)
				m_canTrigger = true;
            break;
...
void Spell::DoSpellHitOnUnit(Unit *unit, const uint32 effectMask)
{
    ...
    if (m_caster != unit)
    {
        ...
        if (!m_caster->IsFriendlyTo(unit))
        {
			...
			if (m_spellInfo->SpellIconID != 2267 && m_spellInfo->Id != 3600 && m_spellInfo->Id != 1725 && m_spellInfo->Id != 36554 && m_spellInfo->Id != 14185 && m_spellInfo->Id != 12323)
				unit->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_HITBYSPELL);
...
void Spell::SearchAreaTarget(std::list<Unit*> &TagUnitMap, float radius, const uint32 type, SpellTargets TargetType, uint32 entry)
{
    ...
    Oregon::SpellNotifierCreatureAndPlayer notifier(*this, TagUnitMap, radius, type, TargetType, entry, pos->GetPositionX(), pos->GetPositionY(), pos->GetPositionZ());
...
void Spell::cast(bool skipCheck)
{
    ...
    if (m_delayMoment && !IsChanneledSpell(m_spellInfo) || m_spellInfo->Id == 14157)
    {
...
uint8 Spell::CanCast(bool strict)
{
    ...
    if (target)
    {
        ...
        if((!IsPositiveSpell(m_spellInfo->Id) && !(m_spellInfo->Id == 7266)) && target->GetTypeId() == TYPEID_PLAYER 
            ...
            && m_caster->ToPlayer()->GetSession()->GetSecurity() < SEC_MODERATOR)
        { // 7266 = duel request, if there is more spells that are blocked by this. i'll prolly remove it.
            // anti faction hack
        ...
        if (target != m_caster)
        {
			...
			// this took me a long time to figure out, also this is the only place where it checks LOS?
            if (!m_IsTriggeredSpell && VMAP::VMapFactory::checkSpellForLoS(m_spellInfo->Id) 
				&& !(spellmgr.GetSpellCustomAttr(m_spellInfo->Id) & SPELL_ATTR_EX_IGNORE_LOS)
        ...
        if (IsPositiveSpell(m_spellInfo->Id))
        {
			...
			if (target->IsImmunedToSpell(m_spellInfo, false) && !target->HasAura(33786, 0)) // cyclone precast fix by bigpwn
                return SPELL_FAILED_TARGET_AURASTATE;
        }
...
uint8 Spell::CheckRange(bool strict)
{
    ...
    if (m_targets.m_targetMask == TARGET_FLAG_DEST_LOCATION && m_targets.m_dstPos.GetPositionX() != 0 && m_targets.m_dstPos.GetPositionY() != 0 && m_targets.m_dstPos.GetPositionZ() != 0)
    {
        ...
        if (dist < min_range)
			return SPELL_FAILED_TOO_CLOSE;
		if (!m_caster->IsWithinLOS(m_targets.m_dstPos.GetPositionX(), m_targets.m_dstPos.GetPositionY(), m_targets.m_dstPos.GetPositionZ())) //Fixing LoS with aoe and remove dnon working elemental freeze range check
			return SPELL_FAILED_LINE_OF_SIGHT;
	}
